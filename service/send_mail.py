...
def send_mails():
    ...
    for mailing in Mailing.objects.filter(mailing_status=Mailing.STATUS_STARTED):
        for mailing_client in mailing.clients.all():
            mailing_log = MailingLogs.objects.filter(log_client=mailing_client, log_mailing=mailing)
            print(mailing_client)
...
def send_email(mailing, mailing_client):
    ...
    try:
        ...
        MailingLogs.objects.create(
            log_status=MailingLogs.STATUS_OK,
        ...
        MailingLogs.objects.create(
            log_status=MailingLogs.STATUS_FAILED,
            log_client=mailing_client,
            log_mailing=mailing,
            response=e
        )
